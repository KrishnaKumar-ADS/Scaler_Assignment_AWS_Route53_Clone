      ...
      <header className="border-b border-zinc-800 bg-zinc-950/80 backdrop-blur sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-6 h-16 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <p className="text-zinc-400 text-sm hidden md:block border-l border-zinc-800 pl-4">{user.email}</p>
            <Button variant="outline" onClick={logout} size="sm" className="border-zinc-800 hover:bg-zinc-900 text-zinc-300">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            <div className="p-6 rounded-2xl border border-zinc-800 bg-zinc-900/40 relative overflow-hidden group hover:border-blue-500/50 transition-colors">
              ...
              <p className="text-4xl font-bold mt-2 text-white">{stats?.total_hosted_zones ?? "-"}</p>
            ...
            <div className="p-6 rounded-2xl border border-zinc-800 bg-zinc-900/40 relative overflow-hidden group hover:border-indigo-500/50 transition-colors">
              <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                <Server className="h-16 w-16 text-indigo-500" />
              </div>
              <h3 className="text-sm font-medium text-zinc-400">Total DNS Records</h3>
              <p className="text-4xl font-bold mt-2 text-white">{stats?.total_dns_records ?? "-"}</p>
              <p className="mt-4 text-xs text-zinc-500">Across all your hosted zones</p>
            </div>
            <div className="p-6 rounded-2xl border border-zinc-800 bg-zinc-900/40 relative overflow-hidden group hover:border-purple-500/50 transition-colors">
              <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                <Activity className="h-16 w-16 text-purple-500" />
              </div>
              <h3 className="text-sm font-medium text-zinc-400">Recent Activity</h3>
              <p className="text-4xl font-bold mt-2 text-white">{stats?.recent_activity_count ?? "-"}</p>
              <p className="mt-4 text-xs text-zinc-500">Audit events in the last 24 hours</p>
            </div>
          </div>
        </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <Button 
                        ...
                        className={newZoneType === "PUBLIC" ? "bg-blue-600 flex-1" : "border-zinc-800 flex-1 text-zinc-400 hover:text-white"}
                      >
                        Public Route
                      </Button>
                      <Button 
                        ...
                        className={newZoneType === "PRIVATE" ? "bg-zinc-700 flex-1" : "border-zinc-800 flex-1 text-zinc-400 hover:text-white"}
                      >
                        Private Route
                      </Button>
                    </div>
            ...
            <Table>
              <TableHeader className="bg-zinc-900/80">
                <TableRow className="border-zinc-800 hover:bg-transparent">
                  <TableHead className="text-zinc-400">Domain Name</TableHead>
                  <TableHead className="text-zinc-400">Type</TableHead>
                  <TableHead className="text-zinc-400">Record Count</TableHead>
              ...
              <TableBody>
                {loading ? (
                  <TableRow>
                    <TableCell colSpan={5} className="text-center py-8 text-zinc-500">Loading zones...</TableCell>
                  </TableRow>
                ) : zones.length === 0 ? (
                  <TableRow>
                  ...
                  zones.map((zone) => (
                    <TableRow key={zone.id} className="border-zinc-800 hover:bg-zinc-800/50 transition-colors group">
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-zinc-400">{zone.status}</span>
                        </div>
                      </TableCell>
